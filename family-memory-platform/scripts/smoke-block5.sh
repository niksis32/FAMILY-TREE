...
if [[ -n "${SEARCH_ID:-}" && "$SEARCH_ID" != "null" ]]; then
  ...
  sleep 1
  curl -sf "${HDR[@]}" "$API/external-archives/searches/$SEARCH_ID" | json_pipe
  echo "OK: archive search poll"
fi

echo ""
echo "[5/12] GET /branding"
curl -sf "${HDR[@]}" "$API/branding" | json_pipe
echo "OK: branding"

echo ""
echo "[6/12] GET /export/templates"
curl -sf "${HDR[@]}" "$API/export/templates" | json_pipe
echo "OK: export templates"

echo ""
echo "[7/12] GET /dna/profile"
HTTP_CODE=$(curl -s -o /tmp/dna_profile.json -w "%{http_code}" "${HDR[@]}" "$API/dna/profile")
cat /tmp/dna_profile.json | json_pipe
echo "OK: dna profile (HTTP $HTTP_CODE)"

echo ""
echo "[8/12] GET /cemetery"
curl -sf "${HDR[@]}" "$API/cemetery" | json_pipe
echo "OK: cemeteries list"

echo ""
echo "[9/12] GET /cemetery/map"
curl -sf "${HDR[@]}" "$API/cemetery/map" | json_pipe
echo "OK: cemetery map"

echo ""
echo "[10/12] POST /cemetery"
CEM_JSON=$(curl -sf -X POST "${HDR[@]}" \
  -H "Content-Type: application/json" \
...
if command -v jq >/dev/null 2>&1; then
  PERSON_ID=$(curl -sf "${HDR[@]}" "$API/persons" | jq -r '.[] | select(.isLiving == false or .deathDate != null) | .id' 2>/dev/null | head -1)
  if [[ -z "$PERSON_ID" || "$PERSON_ID" == "null" ]]; then
    PERSON_ID=$(curl -sf "${HDR[@]}" "$API/persons" | jq -r '.[0].id // empty')
  fi
fi
...
if [[ -n "${CEM_ID:-}" && "$CEM_ID" != "null" && -n "${PERSON_ID:-}" && "$PERSON_ID" != "null" ]]; then
  ...
  BURIAL_JSON=$(curl -sf -X POST "${HDR[@]}" \
    -H "Content-Type: application/json" \
...
# Optional: webhook ping (PROMPT 5-F) — requires PROFESSIONAL+ plan
if [[ "${SMOKE_WEBHOOK_PING:-1}" == "1" ]]; then
  echo ""
  echo "[bonus] POST /webhooks/endpoints + test ping"
  EP_JSON=$(curl -sf -X POST "${HDR[@]}" \
    -H "Content-Type: application/json" \
    -d '{"url":"https://httpbin.org/post","subscribedEvents":["PING"]}' \
    "$API/webhooks/endpoints" 2>/dev/null || echo '{}')
  EP_ID=$(echo "$EP_JSON" | { command -v jq >/dev/null && jq -r '.id // empty' || true; })
  if [[ -n "${EP_ID:-}" && "$EP_ID" != "null" ]]; then
    curl -sf -X POST "${HDR[@]}" "$API/webhooks/endpoints/$EP_ID/test" | json_pipe
    echo "OK: webhook test ping queued"
  else
    echo "WARN: webhook create skipped (plan gate or httpbin unavailable)"
  fi
fi
